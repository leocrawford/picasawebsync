...
def oauthLogin(gd_client):
    ...
    gd_client.SetOAuthInputParameters(gdata.auth.OAuthSignatureMethod.HMAC_SHA1, consumer_key,
    ...
    try:
        ...
        f.close()
        oauth_input_params = gdata.auth.OAuthInputParams(gdata.auth.OAuthSignatureMethod.HMAC_SHA1, consumer_key,
                                                         consumer_secret=consumer_secret)
        # the token key and secret should be recalled from your database
        oauth_token = gdata.auth.OAuthToken(key=key, secret=secret, scopes='https://picasaweb.google.com/data/',
